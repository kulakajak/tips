...
* enabling hstore [link](https://gist.github.com/terryjray/3296171) `sudo psql -d Scuddle_app_development -U orlovic` and `CREATE EXTENSION hstore;` or in one command: `sudo su postgres -c "psql Scuddle_app_test -c 'CREATE EXTENSION hstore;'"` also for both test and development database [link](https://github.com/diogob/activerecord-postgres-hstore/issues/99). If you need to run `rake db:migrate:reset` to rerun all migration and check if db/schema is in sync, than the best way is to alter user to have superuser privil `sudo su postgres -c "psql -d postgres -c 'ALTER USER orlovic WITH SUPERUSER;'"` where orlovic database username (you can see those usernames - roles using pqadmin visual program).
...
* ignore empty values from array (return all except "") is done with `a.reject &:empty?`
